...
export default function Artesao() {
        ...
        async function carregarDadosDoArtesao() {
            ...
            try {
                const [pedidos, avaliacoesRes] = await Promise.all([
                    ...
                    apiService.get(`/avaliacoes?idArtesao=${artesao.id}`).then(res => res.data).catch(() => [])
                ]);

                setCompras(pedidos || []);
                setMinhasAvaliacoes(avaliacoesRes || []);
            } catch (error) {
    ...
    const mediaAvaliacoes = minhasAvaliacoes.length > 0 
        ? (minhasAvaliacoes.reduce((t, a) => t + a.nota, 0) / minhasAvaliacoes.length).toFixed(1) 
        : "4.9";
                            ...
                            <Card.Body pt={4}>
                                <VStack align="stretch" gap={3}>
                                    {minhasAvaliacoes.length > 0 ? (
                                        minhasAvaliacoes.slice(0, 4).map((aval) => (
                                            <Box key={aval.id} p={3} bg="#FAF8F5" borderRadius="md" borderWidth="1px" borderColor="gray.100">
